...
package](package.qmd#creating-a-data-package) already.
...
8.  If you need to update the properties at a later point, you would
    ...
    re-create the properties and write them to the `datapackage.json`
    file.
...
diabetes, and it looks like this:
...
data. Use these extracted properties as a starting point and edit as
needed.
...
resource's properties to the `datapackage.json` file.
...
A data package can contain multiple resources, so their `name` property
must be unique. This `name` property is what will be used later to
create the folder structure for that resource.

The next step is to write the resource properties to the
`datapackage.json` file. Since we included the `resource_properties`
object directly into the `scripts/package_properties.py` file, and since
the `scripts/package_properties.py` file is imported already in
`main.py`, we can simply re-run the `main.py` file and it will update
both the `datapackage.json` file and the `README.md` file.
