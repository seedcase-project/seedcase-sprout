...
data resources, each containing a conceptually
...
For this guide, you will use the
[(fake) data on patients](https://raw.githubusercontent.com/seedcase-project/data/refs/heads/main/patients/patients.csv)
that is already tidy. We've placed this data in a `raw/` folder in the
Data Package and called it `patients.csv`, so that we can keep the
original "raw" data separate from the processed data.
...
need to describe its metadata properties. The
...
3. We have removed saving the properties into `datapackage.json` by
   deleting `write_properties()` to make it easier to see what is new.
   You will learn how to write the resource metadata later in this
   guide.
...
hidden by default on this page. You can click the banner below to show
it.
...
Package
[field type](https://datapackage.org/standard/table-schema/#field-types).
...
object directly into the `SproutProperties` class in the
`scripts/package_properties.py` file, we can add the same steps as in
the last section of the guide to write the `datapackage.json` file.
...
`main.py` file to update the `datapackage.json` file.
