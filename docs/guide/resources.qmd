...
1. The first two lines are what we already leared in the previous section.
2. This is the only new function. It creates the resource properties script.
   Note that the script is only created once, it will not be overwritten if it
   already exists.
3. We temporarily removed the creation of the README file to make it easier to
   see what is new.
...
and optionally the `fields` (i.e., columns
or variables) that you want to include in the resource.
