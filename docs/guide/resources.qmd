...
3.  Add your data to the data batches of the resource.
...
```{python setup}
#| include: false
# This `setup` code chunk creates a package and downloads some raw data.
...
sp.write_file(readme, package_path / "README.md")
...
resource properties from the raw data. This function tries to infer the
data types from the raw data, but it might not get it right, so make
sure to double check the output. It is not possible to infer information
that is not included in the data, like a description of what the data
contains or the unit of the data.
...
in the `package_path` variable. In this guide `package_path` points to a
temporary folder:
...
```{python}
resource_path, _ = sp.create_resource_structure(path=package_path)
...
you the location of the `datapackage.json` file based on the path to
your package.

```{python}
sp.write_resource_properties(
    path=sp.path_properties(package_path),
...
you can list the contents of the resources folder:

```{python}
# TODO: Make a separate `list_resources()` function for this?
print(list(sp.path_resources(package_path).iterdir()))
```
:::
...
Sprout always stores a backup of each addition or modification as a
"batch" file. All the batch data files are stored in a folder called
`batch/` within the resource's folder. They are merged to create the
final Parquet data file for the resource. Batch files can be useful if
you ever need to re-process the data at a later point, troubleshoot any
issues, update incorrect values, or if you need to compare the stored
batch data to your original raw data (the data before it enters into the
data resource).
...
## Building the Parquet resource data file
