...
properties object (`PackageProperties` or
`ResourceProperties`) as input, runs all necessary checks, and raises a group of
`CheckError`s if any checks fail (run `help(CheckError)` for more
details). Each `CheckError` corresponds to a specific violated
requirement. If certain error types are not relevant to your use case,
you can configure the check functions to ignore these.

## Checking resource properties

A set of resource properties with only the required fields filled in
might look like:
...
resource_properties = sp.ResourceProperties(
    ...
    description="Data on the body fat percentage of woolly dormice (*Dryomys laniger*) before and after hibernation. Collected along the Taurus Mountain range in Turkey in 2015.",
)
```

To check that these properties are indeed complete and well-formed, we use the
`check_resource_properties` function. The function will not raise any
errors and will return the original input:

```{python}
resource_properties = sp.check_resource_properties(resource_properties)
pprint(resource_properties)
```

Now, let's say we didn't include a `description` in our resource
...
Finally, let's say there are multiple errors in our resource properties. For
example, we forgot the `description` and gave a `name` containing
...
If we want to check that a set of package properties is well-formed
without considering whether any associated resource properties are
well-formed, we can use the `check_package_properties` function. This
will only run checks on package properties fields and not look at
resource properties fields.

In the package properties below, the required fields `id` and
`description` are missing, and `created` doesn't contain a valid date.
As all three problems relate to fields in the package properties, an
error will be raised for each:
...
However, if the error is in a field belonging to the resource properties
(below, all required fields are missing), it is ignored when the check
runs:
...
package_properties = sp.PackageProperties(
    name="woolly-dormice",
    title="Hibernation Physiology of the Woolly Dormouse: A Scoping Review.",
    description="This scoping review explores the hibernation physiology of the woolly dormouse, drawing on data collected over a 10-year period along the Taurus Mountain range in Turkey.",
    id="123-abc-123",
...
the `check_properties` function. In the properties below, we have three
errors: the required `description` field is missing in both the package
and resource properties, and the resource `path` doesn't point to a data
file. When we run the check, all three errors are listed:
