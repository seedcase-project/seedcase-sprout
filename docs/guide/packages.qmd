...
```{python setup}
#| include: false
import seedcase_sprout.internals as internals
...
is located. Throughout this guide we will refer to the root folder of
the Python project as the root folder of the data package.

Right now, your file structure should look a bit like:

<!-- TODO: Convert the pprint to file trees when listing paths -->

```{python}
#| echo: false
sp.pprint(internals._list_rel_files(package_path))
```
...
data package and the data resources within it. Right now, you don't have
a `datapackage.json` file yet. Before you can create one though, you
need to write out the properties for the data package.

While you could manually create a `datapackage.json` file and then add
the properties manually to it yourself, this is not a pleasant
experience. You would need to know the exact fields and values to fill
in, which are required and which are optional, and how to format them
correctly. To make this easier, Sprout provides a way to create the
properties using Python scripts. So let's make this script first!
...
its resources. Open the `main.py` file in your Python project, delete
everything in it and copy and paste the below code into it.
...
def main():
    ...
    # Simply to have something to return.
    return "Done!"
...
This will then create the `properties.py` file in the newly created
`scripts/` folder of your data package.

::: callout-caution
Because of the way Python scripts and importing works, you should also
create a `__init__.py` file in the `scripts/` folder. In your Terminal,
run the following command to create an empty `__init__.py` file:
...
help documentation for the `PackageProperties` to know what to write.
The core of a data package is its properties. Without which, your data
are simply a collection of files without any context or meaning. The
...
For now, let's write some basic properties for you data package. Below
is an example of what a filled required properties are (including the
optional but highly recommended `contributors` field).
...
create your data package with these properties. You can use the
`write_properties()` function for this. But in order to use it
...
your data package `diabetes-study` folder, which contains the properties
you added to it. Now, you will see the added `datapackage.json` file in
your data package folder.
...
Now you can see that there are more files created in your data package!
...
scripts, updating those scripts and re-running your build pipeline will
