...
some default values in them, but it eventually will contain the
metadata, a.k.a. the `properties`, of your data package. `README.md` is
a prettified, human-readable version of the content of the
`datapackage.json`.

While you *could* manually fill in the details in the `datapackage.json`
file, we have a helper `Properties()` class to make it easier for you.

``` python
properties = sp.Properties.Package(
    title="Diabetes and Hypertension Study",
    description="Data from the 2021 study on diabetes and hypertension",
    contributors=[sp.Properties.Contributors(
        title="Jamie Jones",
        email="jamie_jones@example.com",
        roles=["creator"]
    )],
    licenses=[sp.Properties.Licenses("ODC-BY-1.0")]
)
print(properties)
```
...
Then, to apply this back into the current `datapackage.json` file, you
first use the `create_package_properties()` function:
...
required fields are not filled in to create a valid data package.
:::
...
If you need help with filling in the right properties, check the
documentation for the `Properties()` class or run
`print(sp.Properties.Package())` to get a list of all the fields you can
fill in.
