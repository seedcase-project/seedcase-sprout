...
it's much easier to use the `default()` method of the
`PackageProperties`:
...
learning more about your data package. You can use `as_readme_text()` to
...
maybe project specific, information before writing to the file. Then to
save it to the file:
...
Then you you can use the `update_package_properties()` function while
using the helper `path_properties()` function to point to the
