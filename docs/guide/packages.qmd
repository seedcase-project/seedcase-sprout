...
the properties manually to it yourself, this would require that you
know the exact fields and values to fill
in.
To ease this process, Sprout provides a way to create the
properties using Python scripts. So let's create this script first, by adding it as a step in the `main.py` file!
...
its resources. You can think of it as a pipeline that takes you from beginning to end of creating your data package. Open the `main.py` file in your Python project, delete
...
This will create a `properties.py` file in the newly created
`scripts/` folder of your data package.

::: callout-caution
Because of the way Python scripts and importing works, you should also
create an `__init__.py` file in the `scripts/` folder. You can do this by running the following command in your Terminal:
...
documentation for the `PackageProperties` to know what to write.
...
create your `datapackage.json` file with these properties. You can use the
...
scripts, updating those scripts and re-running your build pipeline (`main.py`) will
