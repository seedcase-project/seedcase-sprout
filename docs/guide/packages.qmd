...
At the core of Sprout is the Data Package, which is a standardized way
to structuring and sharing data. This guide will show you how to create
and manage data packages using Sprout.
...
because you want to create some data packages for studies you are
running. Another example would be if your research group has storage
space on a server that you need to use a Terminal and SSH to access.
:::
...
The first thing you'll need to decide is where you want to store all
your data packages. By default, Sprout will create it in
`~/.sprout/packages/` on Linux (see [Outputs](design/outputs.qmd) for
...
Afterwards, you would create the structure for your first data package
like so:
...
[PosixPath('~/Documents/data-packages/1/datapackage.json'),
 ...
 PosixPath('~/Documents/data-packages/1/resources')]
```

Then to fill in the details in the `datapackage.json` file, you can use
the `create_package_properties()` function:

``` python
package_properties = sp.create_package_properties(
  path=sp.path_package_properties(package_id=1),
  properties=sp.properties_json(
    title="Diabetes and Hypertension Study", 
    description="Data from the 2021 study on diabetes and hypertension", 
    contributors=["Jamie Jones", "Jimmy James"],
    licenses=["ODC-BY-1.0"]
  )
)

# TODO: add an example output of the above.
# Save the package properties back to the file
sp.write_package_properties(
  json_object=package_properties,
  path=sp.path_package_properties(package_id=1)
)
```
...
```         
sprout package create --title "Diabetes and Hypertension Study" \
  --description "Data from the 2021 study on diabetes and hypertension"
```

You will now have a new data package in the location you specified:

```         
~/Documents/data-packages/1/datapackage.json
~/Documents/data-packages/1/README.md
~/Documents/data-packages/1/resources
```
