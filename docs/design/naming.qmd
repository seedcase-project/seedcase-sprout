...
"data resource" to "resource". We use words "property" or "properties"
to describe the metadata about the package or resource that is contained
within the `datapackage.json` file that follows the Frictionless Data
specification.
...
| resource(s) | A single data file within a package. |
...
| template | A template file for specific objects, such as a template properties file. |
| data | The data file within a resource. |
...
| JSON | The main way we pass and save information around. |
| Parquet | A columnar, efficient storage file format that we use to save a data resource. |
...
| copy | Copy a file from on location to another. |
