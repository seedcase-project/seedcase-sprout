...
or via the code-based way (through Python or the command line) in a user-controlled and terminal-only server, or when
...
:   Whenever a specific type of data is stored in the data package, it
    ...
    package, with each data resource in its own folder, e.g. `packages/1/resources/1/`.

    Using the demographics and blood sample example above, the reason
    why these would be considered two separate data resources,
...
:   When a user creates a data resource and adds or updates data in the
    resource, all resource data is processed and stored in
    ...
    database. We use both the Parquet data file and the database
