...
Based on the [naming scheme](naming.qmd), these are the core
...
1.  Whenever `view` is used, it is always for one object (either a
    ...
    always to show basic details of all objects (packages or resources
    in a package). Both `view` and `list` only ever show the information
    contained in the `datapackage.json` and never show actual data (at
    least in a format that might not be in compliance with legal and
    privacy regulations). We don't provide the ability to view data
    directly since it is out of scope for Sprout and we want to minimise
    any security and privacy risks. The IT admin and data package admin
    users can always be able to directly (outside of Sprout) view the
    data as they will have the appropriate IT and legal permissions.
...
3.  `add` only ever adds more data to an existing data resource and not
    to add any new metadata to the `datapackage.json`. Since Sprout
...
Nearly all the of the functions have a `path` argument. Depending on
what the function does, the path object would be different. Use the
...
functions output either a JSON file or path object so it can be easier
to test.

Several of the functions have an optional argument called `properties`,
which is a list of key-value pairs (as a JSON object) that describe the
package that can be used for the `datapackage.json` file and that
...
:   This function lists some basic details contained in the
    ...
    would be allowed by privacy and legal regulations (e.g. like the
...
:   This will show the information contained within the
    ...
    `path_package_properties()` to provide the correct path location.
...
:   This is the second function to use to create a new data package. It
    ...
    using the `properties` argument which must be a JSON object. If the
    `properties` argument isn't given, the user will be prompted to
    enter in the required information needed for the `datapackage.json`
    file. The `properties` argument must follow the Frictionless Data
...
:   Make edits to the `datapackage.json` properties of a specific
    ...
    resources contained within the package. If the `properties` argument
    isn't given, the user will be prompted to edit the required fields.
    The `properties` argument must follow the Frictionless Data
    specification, use `view_package_properties_template()` to get a
    JSON object that follows the correct specification. Use
    `path_package_properties()` in the `path` argument to provide the
...
`delete_package(path, ask)`

:   Completely delete a specific package and all it's data resources.
    Because this action would be permanent, the optional argument `ask`
    would default to true so that the function obtains explicit
    confirmation from the user by prompting the user to confirm yes or
    no on the deletion. Use `path_package()` function in the `path` to
    get the correct location. Outputs `true` if the deletion was
    successful.

`write_package_properties(json_object, path)`

:   Use to write the JSON object back to the `datapackage.json` file.
    The `path` argument is the location of the `datapackage.json` file,
    use the `path_package_properties()` function to provide this path to
...
:   Views the information on a package's specific resource that is
    ...
    `path` and `path_resource_properties()` for the `properties_path()`.
    Outputs a JSON object.

`create_resource_structure(path)`

:   This is the first function to use to setup the structure for a data
    resource. Creates a new data resource in a specific (existing)
    package by creating the file and folder setup as described in the
    [Outputs](outputs.qmd) section. Use the `path_resource()` function
    to provide the correct path location. Creates the files and outputs
    a path object.
...
:   This function either takes the data found in the `data_path`
    ...
    resource specification. The user will be prompted to edit or add any
    fields that the function can't guess (like variable description).
    ...
    Use `path_package()` to provide the correct `path` location. Outputs
    a JSON object only, use `write_resource_properties()` to save back
    to the `datapackage.json` file.

`copy_resource_data_to_raw(path, data_path)`
...
:   Copy the file from `data_path` over into the resource location given
    ...
    file name to store as a backup. See the explanation of this file in
    the [Outputs](outputs.qmd) section. All data in this folder are
    joined together and merged over into the `data.parquet` file using
    the `construct_resource_data()` function. Use `path_resource_raw()`
    to provide the correct `path` location. Copies the file and outputs
    the path object of the created file.

`construct_resource_data(path, raw_path)`

:   This function takes the files provided by `raw_path` and merges them
    into a `data.parquet` file provided by `path`. Use
    `path_resource_data()` to provide the correct path location for
    `path` and `path_resources_raw_files()` for the `raw_path` argument.
...
:   Edit the properties of a resource in a package. The `properties`
    ...
    specification. If the `properties` argument isn't given, the user
    will be prompted to edit the required fields. Use the
    `path_resource_properties()` function to provide the correct path
    location. Outputs a JSON object only, use
    `write_resource_properties()` to save back to the `datapackage.json`
    file.

`delete_resource_raw_file(path, ask)`
...
:   Use this to delete a raw file in the `raw/` folder of a resource.
    ...
    or had an issue during the time when a user first is adding the file
    to a resource. Use `path_resource_raw_files()` to select from the
    list of files found to delete from. Will only delete one file. The
    optional argument `ask` defaults to true so that the function
    obtains explicit confirmation from the user by prompting the user to
    confirm yes or no on the deletion. Outputs `true` if the deletion
    was successful.

`delete_resource_data(path, ask)`

:   Delete all data and raw data of a specific resource. Use
    ...
    associated metadata for the resource.

`delete_resource_properties(path, ask)`

:   Deletes all properties for a resource within the `datapackage.json`
    file. Use `path_resource_properties()` to provide the correct
    location for `path`. The optional argument `ask` defaults to true so
    that the function obtains explicit confirmation from the user by
    prompting the user to confirm yes or no on the deletion. Outputs
    `true` if the deletion was successful.

`write_resource_to_parquet(data, path)`

:   Writes a data object to the `path` location. Use
    `path_resource_data()` to provide the correct path location. Outputs
    the path object of the created file.

`write_resource_to_table(data, path, properties)`
...
`path_resource_properties(package_id, resource_id)`

:   TODO

`path_package_properties(package_id)`
