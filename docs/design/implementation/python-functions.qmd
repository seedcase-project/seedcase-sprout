...
description. Use `path_resources()` to provide the correct path location.
...
and `path_resources_raw()` with `list_files()` for the `raw_files`
...
Use `path_resource_raw()` and `list_files()` to select which file to delete from the
...
<!-- TODO: Not sure if we will keep this database thing -->
:::
