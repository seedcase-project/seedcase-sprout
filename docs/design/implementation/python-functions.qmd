...
Because there already exists an empty `datapackage.json` file, we use
this function to fill in the properties file with details specific to
the package. We update the properties file by using the `properties`
argument and giving it the `PackageProperties` object. See the help
documentation of `PackageProperties` to know what should or needs to be
added to the `datapackage.json` file. Use `path_properties()` to provide
the correct path location. Outputs a JSON object only. Use
`write_package_properties()` to save back to the `datapackage.json`
file.

Whenever you need to edit the `datapackage.json` properties, this is the
function you will use again. It only edits metadata on the package
itself, not on the data resources contained within the package.
...
::: {.callout-note collapse="true"}
### `update_readme(properties_path, readme_path)`
