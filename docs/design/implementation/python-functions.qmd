...
![Diagram showing the internal function flow of the `path_sprout_root()` function.](images/core/path-sprout-root.svg){#fig-path-sprout-root fig-alt="A Plant UML schematic of the detailed code flow within the `path_sprout_root()` function."}
:::

## Property dataclasses

These dataclasses contain an explicit, structured set of official
properties defined within a data package. The main purposes of these
are to use us pass around structured properties objects between
the different functions. It also serves the purpose so that users
can more easily create valid property objects as well as to identify what
fields are available to fill in and include and what are required.
...
Creates a dataclass object with all the necessary properties for a contributor.
This would be given in the `contributors` field of a `PackageProperties` object.
