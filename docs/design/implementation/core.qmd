...
These checks are specific to checking the `datapackage.json`
specification, and not against the data itself.

- `check_properties()`: This is a wrapper function that runs all the
    checks below.
    - `check_properties_against_standard()`: Check if the properties
      given match the requirements of the Frictionless Data Package standard.
    - `check_properties_against_recommendations()`: Check if the
      properties given match the informal recommendations described on the Frictionless Data Package specification. For instance, some properties have descriptions like *"`name` `SHOULD` be human-readable and consist only of lowercase English alphanumeric characters plus `.`, `-` and `_`."*
...
such as checking which would be the property found in the subitem `resources.[].schema.fields.[].constraints` in the `datapackage.json` file.

- `check_data()`:
    - `check_data_against_properties()`:
