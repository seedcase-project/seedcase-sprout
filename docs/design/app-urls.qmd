...
1.  Instead of singular nouns like `package` and `resource`, the URLs
    ...
    all the packages, but `packages/1/` is the folder for the package with the ID of
    1.
...
:   Creates a new resource for data and associated properties. The user
    ...
    basic properties. The user will then need to fill in and complete
    the extracted properties. The user can continue to edit the resource after
    creation.
...
:   Delete all associated data and properties metadata of a specific
    package's specific resource. This is permanent, so the user will be asked to
