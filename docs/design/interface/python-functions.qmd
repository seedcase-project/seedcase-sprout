...
generates a template for the `datapackage.json` file where the user needs
to fill in required fields by providing the `properties` argument using
the helper `PackageProperties` class. Outputs a full Properties object.
Use `write_package_properties()` to store the properties to the
`datapackage.json` file.
:::
...
properties object in a human-readable format. Outputs a text string.
Use `write_text()` to save the text to the `README.md` file.
:::
...
`path` argument is the path to the folder that will have or currently has the `datapackage.json` file. The `path`
argument is to the existing but empty folder or to a folder that doesn't
exist but will be created by this function. Will overwrite the `datapackage.json` file by default if
the file already exists. Returns the same path object as given in the
`path` argument.
:::
