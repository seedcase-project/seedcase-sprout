...
object), built using the `Properties` object, that describe the package
...
generates a template for the `datapackage.json` file that the user needs
...
properties object into a human-readable format. Outputs a text string.
...
`path` argument is the path to the `datapackage.json` file. The `path`
argument is to the existing but empty folder or to a folder that doesn't
exist but will be created by this function. Will overwrite by default if
