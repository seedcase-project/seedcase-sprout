...
Based on the
[naming scheme](/docs/design/architecture/naming.qmd#naming) and the
...
properties argument is a custom `BaseProperties` dataclass (a collection of
...
See the help documentation with `help(SproutProperties())` for more
...
flowchart
    ...
    out[("SproutProperties<br>or Error")]
    in_properties --> function
    function --> out
```
...
on column values (`field.constraints`). See the
[function flow](flows.qmd#flows) page for more details on the internal
