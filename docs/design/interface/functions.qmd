...
For `write_*()` functions, they always will overwrite the file and they
...
### {{< var wip >}} `write_resource_batches(data_path, resource_properties)`

See the help documentation with `help(write_resource_batches)` for
more details.
...
### {{< var wip >}} `read_resource_batches(path)`

Reads in a set of paths to the batch files of a resource and outputs a
list of DataFrames.

```{mermaid}
flowchart
    in_path[/path/]
    function("read_resource_batch_data()")
    out[("List[DataFrame]")]
    in_path --> function
    function --> out
```

### {{< var wip >}} `join_resource_batches(data_list)`

Takes a list of DataFrames and outputs a joined DataFrame. Is
mostly a wrapper around a join function.
...
### {{< var wip >}} `delete_resource_batch(path, confirm)`
