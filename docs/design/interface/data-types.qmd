...
Sprout implements the Frictionless Data Package standard and aims to support the [data types](https://datapackage.org/standard/table-schema/#field-types) it defines. However, Sprout not only describes data with metadata, but also transforms it into a tidy Parquet file that is ready to be queried by the user (see [Outputs](/docs/design/interface/outputs.qmd#files) and [Parquet](https://decisions.seedcase-project.org/why-parquet/) for more details). This means that Sprout supports only data that is compatible (or can be made compatible) with storage in Parquet.

Below we list the Frictionless data types and give a precise definitions for each, as it is used in Sprout. Any discrepancies with the Frictionless definition are highlighted.
...
| `datetime`     | `Datetime` | `timestamp[ms, tz]` | `INT64` (Timestamp(isAdjustedToUTC=true, timeUnit=milliseconds)) |
| `date`     | `Date` | `date32[day]` | `INT32` (Date) |
| `time`     | `Time` | `time64[ns]` | `INT64` (Time(isAdjustedToUTC=false, timeUnit=nanoseconds)) |
| `year`     | `Int32` | `int32` | `INT32` |
| `yearmonth`     | `Date` | `date32[day]` | `INT32` (Date)` |
| `duration`     | `String` | `string` | `BYTE_ARRAY` (String) |
| `geopoint`     | `Array(pl.Float64, 2)` | `fixed_size_list<item: double>[2]` | `FIXED_LEN_BYTE_ARRAY` |
...
: Mapping of Frictionless data types in Sprout.
...
Precision of up to significant 16 digits.

| Name | Allowed values | Examples | 
|--------------|-------|---------|
|decimal indicator|`.`|`12.56`, `50.000`|
|leading sign|`+`(default) or `-`|`12.56`, `+5.00`, `-12`|
|leading and trailing zeros| `0` (optional) |`12.56`, `0012.5600`, `12`|
...
[comment]: <> We could offer timezone conversion very easily. Something like: when there are different timezones, convert to UTC? Or they could set the timezone in the properties and we could convert to that.
