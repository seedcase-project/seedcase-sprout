...
resource, it gets first saved in the `batch/` folder to keep track of additions
or changes. You add the data when:
...
The batch data files are used to keep track of changes to the data. The data
that will be used is kept clean and ready for analysis, while original data is
not deleted. This flow converts batch data into the final resource data file.
...
properties. The `update_resource_batches()` function will update each batch
DataFrame with the new data types or table schema. Then, you can use
`write_resource_batch()` to save the updated data back to the batch files. The
same file names should be used to overwrite old batch files.
...
`write_resource_batch()` function will save the updated data back to the batch
files.
