...
:   These are Python
    ...
    as `SproutProperties` and `ResourceProperties`. While `SproutProperties`
    holds general Data Package properties, we intentionally avoided a name
    such as `DataPackageProperties` to communicate that Sprout uses stricter
    criteria for some properties than the full flexibility that is available
    in the Data Package specification.
