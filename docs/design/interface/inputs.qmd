...
:   Data for a specific data resource that is tidied and staged from the raw
    ...
    data and data ready to be saved into the final data resource clearer. This
...
:   These are [Parquet](https://decisions.seedcase-project.org/why-parquet/)
    files that store data in a structured, columnar format. These files are
    ...
    in the file path that looks like
    `staging/<resource-name>/<timestamp>.parquet`. Staging files in `staging/`
