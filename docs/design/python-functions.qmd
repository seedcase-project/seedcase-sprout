...
`create_resource_properties(properties)`
...
:   This function sets up and structures a new resource property by
    ...
    object). Outputs a JSON object only; use
...
### `extract_resource_properties(path)`

This takes the data found at the `path` location and creates a JSON
...
properties for the specific data resource given. Outputs a JSON object,
use `write_resource_properties()` to save the JSON object to the
`datapackage.json` file.
