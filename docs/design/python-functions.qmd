...
:   This function sets up and structures a new resource property by
    ...
    be given as a JSON object following the Data Package specification
    (use `view_resource_properties_template()` to get a JSON object that follows
    the Frictionless Data Package standard). The `path` argument provides the path to the resource `id`
...
points to the resource `id` folder; use
`path_resource()` to help give the correct path. Outputs a JSON object.
Use `write_resource_properties()` to
