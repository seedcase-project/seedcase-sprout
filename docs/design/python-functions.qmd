...
This function either takes the data found in the `data_path` location
and creates a JSON object following the Frictionless Data standard or
takes the `properties` argument to fill in the correct fields for the
data resource in the `datapackage.json` file.

If the `data_path` argument is given, it tries to guess and fill in some
of the property fields required by the Frictionless Data resource
specification. Use `edit_resource_properties()` to fill in any missing
fields.

If the `data_path` argument is not given, the `properties` argument is
required and is used to directly provide the properties values for the
resource in the `datapackage.json` file. It must be given a a JSON
object (use `view_resource_properties_template()` to have the correct
JSON object).

Use `path_resource()` to provide the correct `path` location or use the
output of `create_resource_structure()`. Outputs a JSON object only; use
